...
class ExponentialSchedule(Schedule):
    """
    Exponential decay between initial_p and final_p over schedule_timesteps.
    After this many timesteps pass, final_p is returned.

    :param schedule_timesteps: (int) Number of timesteps for which to linearly anneal initial_p to final_p
    :param initial_p: (float) initial output value
    :param final_p: (float) final output value
    :param order: (integer) order of Taylor series approximation
    """

    def __init__(self, schedule_timesteps, final_p, initial_p=1.0, order=5):
        self.schedule_timesteps = schedule_timesteps
        self.final_p = final_p
        self.initial_p = initial_p
        self.order = order
        self.gradient = np.exp(self.initial_p)

    def value(self, step):
        f_x = np.exp(self.initial_p)
        for i in range(self.order):
            n = i + 1
            f_x += self.gradient * (1 / factorial(n)) * ((step / self.schedule_timesteps) * (self.final_p - self.initial_p)) ** n
        return f_x
